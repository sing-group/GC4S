...
// numerical values with two conditions and forty samples. In this case, a
// Student's t-test (implemented by the StudentsTTest class) is used to
// compute the p-values.
public class TwoConditionsMeasurementsTableDemo {
	public static void main(String[] args) {
		...
		// the table. GC4S provides different t-test implementations for
		// numerical values, but it is possible to provide custom tests by
		// implementing Test<Number> (or Test<Integer>, Test<Double>, and so
		// on).
		Test<Number> test = new StudentsTTest();
		...
		table.setHighlighters(
			...
			new NumberHighlighter(table, BLUE, ORANGE)
		);
